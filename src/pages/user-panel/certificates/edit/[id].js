...
export default function AddAds() {
    ...
    const breadcrumbs = [
        <Link className={"text-decoration-none"} underline="hover" key="1" color="inherit" href={"/user-panel/certificates/1"}>
            گواهینامه ها
        </Link>,
    ...
    const dataFetch = async () => {
        const res = await fetch(`${process.env.LOCAL_URL}/api/user-panel/certificates/edit/${router.query.id}`)
    ...
    const submitHandler = async (event) => {
        ...
        if (titleError || statusError) {
            ...
            try {
                const res = await axios.put(`${process.env.LOCAL_URL}/api/user-panel/certificates/edit/${router.query.id}`, formData, {
                        headers: {
                ...
                if (res.data.status) {
                    ...
                    await Swal.fire({
                        icon: 'success',
                        text: "اطلاعات گواهینامه تغییر یافت",
                    })
                    await router.push("/user-panel/certificates/1")
                } else {
                    Nprogress.done()
                    await Swal.fire({
                        icon: 'error',
                    ...
                    <form>
                        <div className={"d-flex flex-column align-items-center gap-3 py-5"}>
                            ...
                            <div
                                ...
                                <label className={"mt-5"}>در صورت تمایل به تغییر ٬ عکس مورد نظر را وارد
                                    کنید</label>
