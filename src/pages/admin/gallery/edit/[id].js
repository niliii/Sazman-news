...
export default function AddAds() {
    ...
    const breadcrumbs = [
        <Link className={"text-decoration-none"} underline="hover" key="1" color="inherit" href={"/admin/gallery/1"}>
            گالری عکس
        </Link>,
    ...
    const dataFetch = async () => {
        const res = await fetch(`${process.env.LOCAL_URL}/api/admin/gallery/edit/${router.query.id}`)
    ...
    const submitHandler = async (event) => {
        ...
        if (titleError || statusError) {
            ...
            try {
                const res = await axios.put(`${process.env.LOCAL_URL}/api/admin/gallery/edit/${router.query.id}`, formData, {
                        headers: {
                ...
                if (res.data.status) {
                    ...
                    await Swal.fire({
                        icon: 'success',
                        text: "اطلاعات عکس تغییر یافت",
                    })
                    await router.push("/admin/gallery/1")
                } else {
                    Nprogress.done()
                    await Swal.fire({
                        icon: 'error',
                                ...
                                </span>
                                {
                                    DATA.status ?
                                        <img className={"col-md-9 col-11"} alt={""}
                                             src={`${process.env.SERVER_URL}/${DATA.data.file}`}/>
                                        :
