...
export default function AddAds() {
    ...
    const breadcrumbs = [
        <Link className={"text-decoration-none"} underline="hover" key="1" color="inherit" href={"/admin/certificates/1"}>
            گواهینامه ها
        </Link>,
    ...
    const dataFetch = async () => {
        const res = await fetch(`${process.env.LOCAL_URL}/api/admin/certificates/edit/${router.query.id}`)
    ...
    const submitHandler = async (event) => {
        ...
        if (titleError || statusError) {
            ...
            try {
                const res = await axios.put(`${process.env.LOCAL_URL}/api/admin/certificates/edit/${router.query.id}`, formData, {
                        headers: {
                ...
                if (res.data.status) {
                    ...
                    await router.push("/admin/certificates/1")
                } else {
                    Nprogress.done()
                    await Swal.fire({
                        icon: 'error',
