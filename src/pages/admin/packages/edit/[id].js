...
export default function Add({data}) {
    ...
    const [postsCount, setPostsCount] = useState(data.data.posts_count)
    const [postsCountError, setPostsCountError] = useState(false)

    const [vipPostsCount, setVipPostsCount] = useState(data.data.vip_posts_count)
    const [vipPostsCountError, setVipPostsCountError] = useState(false)
    ...
    const [state, setState] = useState({
        catalog: data.data.permissions.some(item => item.id === 3),
        gallery: data.data.permissions.some(item => item.id === 2),
        certificate: data.data.permissions.some(item => item.id === 1),
    });
    ...
    const submitHandler = async (event) => {
        ...
        if (nameError || postsCountError || vipPostsCountError) {
            ...
            try {
                const res = await fetch(`${process.env.LOCAL_URL}/api/admin/packages/${router.query.id}`, {
                    method: "PUT",
                    body: JSON.stringify({
                        _method : "PUT",
                        title: name,
...
export async function getServerSideProps(context) {
    ...
    const dataResponse = await fetch(`${process.env.SERVER_URL}/panel/packages/${params.id}`, {
        method: "GET",
        headers: {
