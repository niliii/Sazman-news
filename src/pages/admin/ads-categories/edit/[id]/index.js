...
export default function Edit({data}) {
    ...
    const submitHandler = async (event) => {
        ...
        if (nameError) {
            ...
            try {
                const res = await fetch(`${process.env.LOCAL_URL}/api/admin/ads-categories/edit/${router.query.id}`, {
                    method: "PUT",
                    body: JSON.stringify({
                        title: name,
                        status: status
...
export async function getServerSideProps(context) {
    ...
    const dataResponse = await fetch(`${process.env.SERVER_URL}/panel/ads_categories/${params.id}`, {
        method: "GET",
        headers: {
